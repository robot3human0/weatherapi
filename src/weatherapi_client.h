...
class WeatherApiClient {
  public:
    ...
    WeatherApiClient(const std::string key) : client_("https://api.weatherapi.com"), key_(key) {}
    ...
    std::string MakeRequest(Weather_Request_T& req) {
      ...
      std::string request_str = ("/v1" 
                                 + API_Method_T::MethodAsString(req.method) 
                                 + (req.xml ? ".xml" : ".json")
                                 + "?key=" 
                                 + key_ 
                                 + parameters
  ...
  private:
    std::string GetParametersString(Weather_Request_T& req) {
      ...
      if (req.method == API_Method_T::kForecast || req.method == API_Method_T::kHistory) {
        if (req.method == API_Method_T::kForecast) {
          if (req.days)         { result += req.CheckDays();                  }
        ...
        if (!req.end_dt.empty() && dt_flag)          { result += "&end_dt="     + req.end_dt;     }
